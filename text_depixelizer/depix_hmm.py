...
from text_depixelizer.parameters import PictureParameters, TrainingParameters, LoggingParameters, PiximageParameters,\
...
def depix_hmm(picture_parameters: PictureParameters,
    ...
    # pickled_font = pickle.dumps(picture_parameters.font, protocol=pickle.HIGHEST_PROTOCOL)

    if img_path:
        # 加载模型
        with open(f'arial_50_blocksize-{picture_parameters.block_size}.pkl', 'rb') as f:
            hmm = pickle.load(f)
        print('开始对马赛克图片进行识别')
    ...
    # 保存模型到文件
    with open(f'arial_50_blocksize-{picture_parameters.block_size}.pkl', 'wb') as f:
        pickle.dump(hmm, f)
        print("模型保存成功")
