...
def determine_origin(padding: Tuple[int, int], font_metrics: Tuple[int, int], offset: Tuple[int, int], block_size:  int) -> Tuple[int, int]:
    origin_x: int = padding[0] - offset[0] # 设置起点
    tiles_y_above_baseline: int = math.ceil((font_metrics[0] - offset[1]) / block_size)
    # print(f"tiles_y_above_baseline{tiles_y_above_baseline}=font_metrics[0]{font_metrics[0]}-{offset[1]}/block_size{block_size}")
    origin_y: int = padding[1] + font_metrics[0] - (offset[1] + tiles_y_above_baseline*block_size)
    # print(f"origin_y{origin_y}=padding[1]{padding[1]}+font_metrics[0]{font_metrics[0]}-offset[1]{offset[1]}+tiles_y_above_baseline*20")
...
def pixelize_image(original_image: OriginalImage, pixelization_options: PixelizationOptions) -> Image:  # 打码
    ...
    origin: Tuple[int, int] = determine_origin(  # 修改origin
...
def pixelize_area(image: Image, block_size: int, origin: Tuple[int, int], n_tiles: Tuple[int, int]) -> Image:
    ...
    for i in range(n_tiles[0]):
        for j in range(n_tiles[1]):
            ...
            # print(f"打码区块({left},{right}),({top},{bottom})")
    # 裁剪图片
    # print("origin[0],origin[1]",origin[0],origin[1])
    cropped_image = pixelized_image.crop((origin[0], origin[1], origin[0]+n_tiles[0]*block_size, origin[1]+(n_tiles[1])*block_size))
    cropped_image.save("/Users/lianghaiyan/Desktop/pic/cropped_image.png")
