...
def generate_image_from_text(text: str, options: ImageCreationOptions) -> OriginalImage:
    ...
    image_size: Tuple[int, int] = (2*options.padding[0] + width, 2*options.padding[1] + ascent + descent)  # 生成图片大小
    # print("image_size", image_size)

    # 改动：删除padding
    # width, height = options.font.getsize(text)
    # image_size: Tuple[int, int] = (2*options.padding[0] + width, 2*options.padding[1]+height)

    # print("ascent", ascent)  # 上下限高度、训练集图片大小
