...
from fonts import DemoFontPaths
import numpy as np
...
class DepixHMM(HMM):
    ...
    #     print(state)
    #     # 删除不可序列化的字体对象
    #     del state['picture_parameters']
    #     return state
    #
    # def __setstate__(self, state):
    #     self.__dict__.update(state)
    #     # 根据字体名称或路径重新创建字体对象
    #     self.picture_parameters: PictureParameters = PictureParameters(
    #         pattern=r'\d{9}',
    #         window_size=1,
    ...
    def train(self):
        ...
        # print("windows_train_flattened", windows_train_flattened)
        # print("windows_train.windows", windows_train[0])
        # print("------------------")
    ...
    def calculate_hmm_properties(self, windows_train: List[Window]):
        ...
        # Compute the probability matrices    计算概率矩阵
    ...
    def test_image(self, img: Image):
        """
        Takes a pixelized image and reconstructs the hidden string  输出字符串
        """
        ...
        n_tiles: Tuple[int] = tuple(int(el / block_size) for el in img.size)  # 计算n_tiles马赛克块数
        ...
        for window_index in range(n_tiles[0] - window_size + 1):
            ...
            # print(np.asarray(img).shape)  # 马赛克图片大小
    ...
    def test_windows(self, windows: List[Window]) -> str:
        """
        Takes a list of clustered windows and returns the most likely sequence of characters  获取窗口预测的字符
        """
    ...
    def evaluate(self) -> Tuple[float, float]:
        """
        Generates test data and checks it with the already trained model. Returns two values:   测试
    ...
    def get_emission_probabilities(windows: List[Window], states: List[Tuple[str, ...]],
        ...
        emission_probabilities: np.ndarray = emission_probabilities_unnormalized / emission_probabilities_unnormalized.sum(
            axis=1)[:, np.newaxis]
    ...
    def print_states(self):
        ...
        for i in range(1, max_state_length + 1):
            ...
            logging.warning(
                f'Found {len(states_with_length_i)} states with length {i}, expected {math.pow(len(unique_characters), i)}')
